...
import { PROFESSIONAL_STYLES, BASE_RESET } from './styles';
import { MAX_BULLETS_PER_ROLE } from '../constants';

const s = PROFESSIONAL_STYLES;
...
function sectionHeading(label: string): string {
  return `
    <div style="border-bottom: 2px solid ${s.primaryColor}; margin-bottom: 12px; padding-bottom: 4px;">
      <h2 style="font-size: ${s.sectionHeadingSize}; font-weight: ${s.headingWeight}; color: ${s.primaryColor}; text-transform: uppercase; letter-spacing: 1.5px;">
        ${label}
      </h2>
    </div>
  `;
...
function renderHeader(data: ResumeData): string {
  const photo = data.photoUrl ? `
    <img src="${data.photoUrl}" style="width: 90px; height: 110px; object-fit: cover; border-radius: 4px; border: 2px solid rgba(255,255,255,0.3); float: right; margin-left: 16px;" />
  ` : '';
  return `
    <header style="background: ${s.primaryColor}; color: #ffffff; padding: 28px 44px; margin: 0 -44px 32px -44px; overflow: hidden;">
      ${photo}
      <h1 style="font-size: ${s.nameSize}; font-weight: ${s.headingWeight}; letter-spacing: -0.5px; margin-bottom: 6px;">
        ${escapeHtml(data.name)}
      </h1>
      <p style="font-size: ${s.roleSize}; opacity: 0.9; margin-bottom: 4px;">
        ${escapeHtml(data.currentRole)} &nbsp;·&nbsp; 경력 ${data.totalYearsExp}년
      </p>
      ${data.email ? `<p style="font-size: ${s.bodySize}; opacity: 0.75; margin-top: 4px;">${escapeHtml(data.email)}</p>` : ''}
      ${data.targetCompany ? `<p style="font-size: ${s.bodySize}; opacity: 0.75; margin-top: 4px;">지원: ${escapeHtml(data.targetCompany)}${data.targetPosition ? ` · ${escapeHtml(data.targetPosition)}` : ''}</p>` : ''}
    </header>
  `;
}
...
function renderExperience(exp: ResumeExperience, index: number): string {
  ...
  return `
    <div class="resume-exp-item" style="${index > 0 ? `margin-top: ${s.itemGap}; padding-top: ${s.itemGap}; border-top: 1px solid ${s.borderColor};` : ''}">
      <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 3px;">
        <div>
          <span style="font-size: 14px; font-weight: ${s.headingWeight}; color: ${s.primaryColor};">${escapeHtml(exp.company)}</span>
          <span style="font-size: ${s.bodySize}; color: ${s.secondaryColor}; margin-left: 10px; font-weight: 500;">${escapeHtml(exp.role)}</span>
        </div>
      ...
      ${exp.techStack.length ? `
        <p style="font-size: 12px; color: ${s.secondaryColor}; margin-bottom: 6px; font-weight: 500;">
    ...
    <section style="margin-bottom: ${s.sectionGap};">
      ...
      ${Object.entries(byCategory).map(([category, catSkills]) => `
        <div style="margin-bottom: 8px; display: flex; gap: 12px; align-items: baseline;">
          <span style="font-size: 12px; color: ${s.secondaryColor}; font-weight: 600; min-width: 80px; flex-shrink: 0;">${escapeHtml(category)}</span>
...
function renderStrengths(strengths: string[]): string {
  ...
  return `
    <section style="margin-bottom: ${s.sectionGap};">
      ...
      <ul style="list-style: none; padding: 0;">
        ${strengths.map(str => `
          <li style="font-size: ${s.bodySize}; color: ${s.textColor}; padding: 3px 0; display: flex; gap: 8px; break-inside: avoid;">
            <span style="color: ${s.secondaryColor};">${s.bulletStyle}</span>
...
export function renderProfessional(data: ResumeData): string {
  return `<!DOCTYPE html>
<html lang="ko">
<head>
  <meta charset="UTF-8">
