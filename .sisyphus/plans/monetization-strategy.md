...
| 결제 | 없음 | Stripe (런칭 시) -> PortOne (포스트 런칭) |
| 가격 | 무료 | Free / Pro |
...
- 타임라인을 현실적 41주로 확장
- 결제를 Stripe 단독으로 런칭, PortOne은 포스트 런칭으로 연기
...
[Free 티어]
  ...
  월 최대: 3세션 x 10질문 = 30질문
  사용자당 월 최대 비용: 30 x $0.006 = $0.18
  실제 사용량 (40%): 12질문 x $0.006 = $0.072
  수익: $0
  → 1,000 무료 사용자 = -$72/월

[Pro 티어]
  모델: Haiku 4.5 ($0.006/질문)
  월 평균: ~50세션 x 30질문 = 1,500질문 (실제 40%: 600질문)
  사용자당 월 비용: 600 x $0.006 = $3.60
  최대 비용 (전량 사용 시): 1,500 x $0.006 = $9.00
  수익: ₩24,900 (~$19)
  → 실제 마진: +$15.40/사용자/월
  → 최대 사용 시에도 마진: +$10/사용자/월
```
...
| # | 리스크 | 영향 | 확률 | 대응 |
|---|--------|------|------|------|
| R1 | **AI 비용 폭주** - Free 티어 사용자 급증 시 AI 비용이 수익을 초과 | 사업 지속성 위협 | 높음 | Free 티어에 Haiku 강제, 월 3세션 하드캡, 비용 일일 모니터링, 킬스위치(무료 가입 중단) 준비. 2026 가격 기준 최대 $0.18/무료사용자/월. |
...
일일 확인:
  - AI 비용 / 매출 비율 (목표: < 40%)
  - 사용자당 평균 AI 비용 (목표: Free < $0.18, Pro < $9)
  - 웹훅 실패율 (목표: < 0.1%)
  - 에러율 (목표: < 1%)
  - Redis 가용성
